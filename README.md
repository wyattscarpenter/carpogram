...
Ain't no complex ZWJ emoji sequences in carpogram because lmao are you crazy those are just more characters just encode them my guy. In fact, eg, we will be encoding all of the country flags present in unicode via the regional indicator hack as newly distinct characters. We will never have a problem where the glyph of the flag of afghanistan should change retroactively, since the flag thus encoded will not "just be the name of the country", but a specific character that is supposed to look a particular way. Incidentally, this is another example where it is difficult to automatically convert from unicode to carpogram. When the user input the symbol(s) 🇫🇷, what did they see? What did they expect to see? What did they mean? What does your information system now assume it's equivalent to? We hope to avoid problems like this forward-going, but we cannot do so for the past.
...
The idea of having an izer izer and then an i and then the letter in question initially struck me as a "cute" idea like people used to do in computing but not suitable for serious work these days. But... maybe...?
...
• As above, but with diacritical marks also ignored. This is useful for allowing english users to search for foreign terms, etc. This is known as "cowboy equality".
• Various domain-dependent schemes like ignoring whitespace; considering only alphanumerics; in extremis only considering length; etc.

The terms "equality" and "equivalence" are synonymous for these purposes, with "equality" understood to mean "equality [under these rules]".
